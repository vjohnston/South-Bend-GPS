...
int main (int argc, char *argv[]) {
  ...
  test_equal(m.route3(su, tu, distance), true);
  test_close(distance, 1.86506);

  m.geocode("620 W Washington St", su, sv, tpos);
  m.geocode("4477 Progress Dr", tu, tv, tpos);
  test_equal(m.route3(su, tu, distance), true);
  test_close(distance, 3.45776);

  m.geocode("4477 Progress Dr", su, sv, tpos);
  m.geocode("1417 E Wayne St", tu, tv, spos);
  test_equal(m.route3(su, tu, distance), true);
  test_close(distance, 5.11051);

  test_equal(m.route3(422740742, 408927857, distance), false);
